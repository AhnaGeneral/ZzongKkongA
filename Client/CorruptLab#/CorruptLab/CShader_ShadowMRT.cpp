...
void Shader_ShadowMRT::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void Shader_ShadowMRT::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void Shader_ShadowMRT::ReleaseShaderVariables()
{
}

D3D12_SHADER_BYTECODE Shader_ShadowMRT::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"HLSL_ShadowTarget.hlsl", "VSShdaowTarget", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE Shader_ShadowMRT::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"HLSL_ShadowTarget.hlsl", "PSShadowTargeet", "ps_5_1", ppd3dShaderBlob));
}

void Shader_ShadowMRT::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
...
void Shader_ShadowMRT::ReleaseObjects()
{
	if (m_pd3dGraphicsRootSignature) m_pd3dGraphicsRootSignature->Release();

	if (m_pTextures)m_pTextures->ReleaseUploadBuffers();
